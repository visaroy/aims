...
`environment` block), pushes the start commit. `--continues-from` records an optional validated predecessor session ID; it does not replace normal handoff/adopt of the same session branch.
...
merged/absent session; warns on a recent (possibly live) writer unless `status=handoff`. Older sessions without a handoff boundary remain adoptable and report that detailed delta is unavailable.
