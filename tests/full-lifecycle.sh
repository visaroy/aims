...
[ "$("$ENGINE" version)" = "$(tr -d '\r\n' < "$ENGINE_ROOT/VERSION")" ] || { echo "Engine and VERSION do not match" >&2; exit 1; }
LAB="$(mktemp -d "${TMPDIR:-/tmp}/aims-acceptance.XXXXXX")"
cleanup() { [ "${AIMS_KEEP_LAB:-0}" = 1 ] && { printf 'Preserved test directory: %s\n' "$LAB"; return; }; rm -rf "$LAB"; }
trap cleanup EXIT
trap 'exit 130' INT; trap 'exit 143' TERM HUP
printf 'Disposable test directory: %s\n' "$LAB"
ORIGIN="$LAB/origin.git"; HOME_A="$LAB/home-a"; HOME_B="$LAB/home-b"; DATA_A="$HOME_A/.aims"; DATA_B="$HOME_B/.aims"
mkdir -p "$HOME_A/.config" "$HOME_B/.config"
export GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_NOSYSTEM=1 AIMS_GIT_NAME="AIMS Acceptance" AIMS_GIT_EMAIL="aims@example.invalid"
...
if git --git-dir="$ORIGIN" show-ref --verify --quiet "refs/heads/ai/$SESSION_ID"; then echo "Session branch still exists after publish" >&2; exit 1; fi
