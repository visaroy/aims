...
REMOTE="$TMP/origin.git"; DATA="$TMP/data"; SID="20260727T000001Z-meta-by-id-test-hermes"; BRANCH="ai/$SID"; WT="$DATA/.worktrees/$SID"
...
output="$(cd "$DATA/.worktrees" && AIMS_HOME="$DATA" "$ENGINE_ROOT/bin/aims" handoff "$SID")"
printf '%s\n' "$output" | grep -F "OK: HANDOFF $SID"
...
test "$(git -C "$DATA" show "origin/$BRANCH:recovery-note.txt")" = 'unsaved work'
test -d "$WT"
printf 'PASS: handoff accepts a session id from .worktrees\n'
