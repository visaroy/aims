...
out="$(AIMS_HOME="$DATA" "$ENGINE_ROOT/bin/aims" start meta continuation hermes --scope path:continuation --continues-from 20260728T000001Z-meta-source-hermes)"
...
PY
if AIMS_HOME="$DATA" "$ENGINE_ROOT/bin/aims" start meta bad hermes --scope path:bad --continues-from '../unsafe' >/dev/null 2>&1; then echo 'expected unsafe predecessor rejection' >&2; exit 1; fi
